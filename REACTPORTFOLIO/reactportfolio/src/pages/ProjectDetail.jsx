...
function ProjectDetail() {
  ...
  const defaultProjects = [
    ...
    { _id: '9', title: 'Jolofera', description: 'Site e-commerce moderne pour la vente de produits alimentaires et cosmétiques naturels africains. Interface responsive avec panier et paiement intégré.', technologies: ['React', 'Vite', 'CSS'], githubUrl: 'https://github.com/Amethnb2218/flashrv-react', liveUrl: 'https://www.jolofera.com/', imageUrl: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="600" height="360"><rect width="600" height="360" fill="#1a1510"/><rect x="180" y="80" width="240" height="160" rx="12" fill="#1e293b" stroke="#f59e0b" stroke-width="1.5" opacity="0.8"/><text x="300" y="170" text-anchor="middle" fill="#f59e0b" font-family="monospace" font-size="16">Jolofera</text><text x="300" y="320" text-anchor="middle" fill="#94a3b8" font-family="monospace" font-size="14">E-commerce + React</text></svg>') },
    { _id: '10', title: 'Teranga AI Assistant', description: "Assistant intelligent propulsé par l'IA pour répondre aux questions sur le Sénégal. Interface conversationnelle moderne avec historique des échanges.", technologies: ['React', 'Node.js', 'AI/LLM'], githubUrl: 'https://github.com/Amethnb2218/teranga-ai', liveUrl: 'https://teranga-assistant.onrender.com/', imageUrl: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="600" height="360"><rect width="600" height="360" fill="#12101e"/><rect x="180" y="70" width="240" height="200" rx="16" fill="#1e293b" stroke="#8b5cf6" stroke-width="1.5" opacity="0.8"/><text x="300" y="180" text-anchor="middle" fill="#8b5cf6" font-family="monospace" font-size="16">Teranga AI</text><text x="300" y="340" text-anchor="middle" fill="#94a3b8" font-family="monospace" font-size="14">AI Chat + Node.js</text></svg>') },
    { _id: '11', title: 'Frescoop UEMOA', description: 'Plateforme web collaborative pour les coopératives de la zone UEMOA. Gestion des membres, transactions et reporting.', technologies: ['React', 'Node.js', 'MongoDB'], githubUrl: 'https://github.com/seydinalimamoulayeyade/frescoopuemoa-v2', liveUrl: 'https://frescoop-web-icts-wbfo.onrender.com/', imageUrl: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="600" height="360"><rect width="600" height="360" fill="#0f1a12"/><circle cx="250" cy="150" r="30" fill="none" stroke="#10b981" stroke-width="1.5"/><circle cx="350" cy="150" r="30" fill="none" stroke="#10b981" stroke-width="1.5"/><circle cx="300" cy="220" r="30" fill="none" stroke="#10b981" stroke-width="1.5"/><text x="300" y="310" text-anchor="middle" fill="#94a3b8" font-family="monospace" font-size="14">React + Node.js + MongoDB</text></svg>') }
  ]
  ...
  useEffect(() => {
    ...
    fetch(`/api/projects/${id}`, { signal: controller.signal })
      ...
      .then((data) => {
        ...
        if (data && data.title) {
          if (!data.imageUrl) {
            const match = defaultProjects.find(d => d.title === data.title)
            if (match && match.imageUrl) data.imageUrl = match.imageUrl
          }
          setProject(data)
        } else {
